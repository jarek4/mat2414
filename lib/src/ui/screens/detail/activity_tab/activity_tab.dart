...
import 'activity_tab_state.dart';
...
class ActivityTab extends StatelessWidget {
  ...
  Future<void> _handleAddNewActivityBtn(BuildContext context) async {
    final ActivityTabState state = Provider.of<ActivityTabState>(context, listen: false);
  ...
  Widget build(BuildContext context) {
    return Column(mainAxisSize: MainAxisSize.min, children: [
      ...
      Expanded(
        ...
        stream: Provider.of<ActivityTabState>(context).watchDayActivities().distinct(),
        ...
        builder: (BuildContext context, AsyncSnapshot<UnmodifiableListView<Activity>?> snapshot) {
          ...
          if (isActive && snapshot.hasData && (snapshot.data == null || snapshot.data!.isEmpty)) {
            final date = dateFormatter(context.read<ActivityTabState>().selectedDate.date, 'd/MM');
            return Center(
              ...
              itemBuilder: (context, i) {
                return ShimmerLoading(
                  ...
                  child: DismissibleActivityListItem(
                    key: data.isNotEmpty
                        ? ObjectKey(data[i])
                        : ObjectKey(ConstantValues.emptyActivity),
                    item: data.isEmpty ? ConstantValues.emptyActivity : data[i],
                    onDelete: () => context.read<ActivityTabState>().deleteActivity(data[i].id),
